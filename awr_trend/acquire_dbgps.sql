...
REM
REM     Purpose:
REM       You can find out metric_name "DB Block Gets Per Sec" and metric_unit "Blocks Per Second" in the view "v$metric".
REM       Typically there saves the average value of "DB Block Gets Per Sec" in each of snap_id of the SDDV (Static Data
REM       Dictionary View), "DBA_HIST_SYSMETRIC_SUMMARY", (in which the value of its column "metric_name" is
REM       "DB Block Gets Per Sec" and "metric_unit" is "Blocks Per Second"), here we use the analytic function "LAG () OVER()"
REM       to get the prior snap_id from current snap_id for more clearly showing "DB Block Gets Per Sec" between these two snap_id.
REM
REM       SET LINESIZE 80
REM       DESC acquire_db_block_gets_per_sec
REM        Name                                      Null?    Type
REM        ----------------------------------------- -------- ----------------------------
...
COLUMN dbgps       FORMAT 999,999.99
...
FROM (
       SELECT instance_number
            ...
            , metric_name
            , metric_unit
            , ROUND(average, 2) dbgps
       FROM dba_hist_sysmetric_summary
       WHERE metric_name = 'DB Block Gets Per Sec'
